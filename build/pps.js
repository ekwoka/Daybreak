(async function () {
  ...
  window.Daybreak.ppsSubmit = async function (el) {
    ...
    if (el.id == "submit")
      value = encodeURIComponent(
        el.closest(".content-box").querySelector("#freeText").value
      );
    if (value === "") return;
    disableButtons(el);
  ...
  async function postAnswer(value) {
    await fetch(
      `https://maker.ifttt.com/trigger/${survey.ifttt.event}/with/key/${survey.ifttt.key}?value1=${survey.qId}&value2=${value}`,
  ...
  function multiChoice() {
    lines.push(
      [
        ...
        ...survey.options.map(
          (l) => `<button class="btn-primary">${l}</button>`
        ),
        "</div>"
      ].join("")
    );
